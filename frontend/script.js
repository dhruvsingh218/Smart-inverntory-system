...
async function onScanSuccess(decodedText, decodedResult) {
    // 1. Input box mein value dikhayein
    document.getElementById('barcodeInput').value = decodedText;

    // 2. Backend se data maangein
    try {
        let response = await fetch(`/api/get-product/${decodedText}/`);
        let data = await response.json();

        if (data.success) {
            // Agar product mil gaya, toh cart mein add karein
            addToCart(data);

            // Success sound ya chota alert (Optional)
...
// Yeh function Table mein Row add karega
function addToCart(product) {
    let tableBody = document.querySelector(".cart-table-body"); // Note: Apne table body ko class/id de dena

    // Check karein agar HTML mein table body nahi hai toh error na aaye
    if (!tableBody) {
        console.log("Table body not found! Please add class 'cart-table-body' to your <tbody> tag.");
        return;
    }
    ...
    let newRow = `
        <tr class="border-b">
            ...
            <td class="p-2">
                <input type="number" value="${qty}" class="w-12 border rounded text-center">
            </td>
            <td class="p-2 font-bold">₹${total}</td>
            <td class="p-2 text-red-500 cursor-pointer" onclick="this.parentElement.parentElement.remove()">X</td>
        </tr>
    `;

    // Table ke andar naya row daal dein
    tableBody.insertAdjacentHTML('beforeend', newRow);

    // Total Amount update karne ka logic yahan aayega
}

// Is wrapper ka matlab: "Jab tak poora page load na ho jaye, tab tak wait karo"
document.addEventListener('DOMContentLoaded', (event) => {

    // Aapka scanner code yahan paste karein
...
async function saveProductToBackend() {
    // 1. Form se values nikalo
    const name = document.getElementById('productName').value;
    const category = document.getElementById('productCategory').value;
    const code = document.getElementById('productID').value; // Barcode scanner se jo aayega
    const price = document.getElementById('productPrice').value;
    const stock = document.getElementById('productStock').value;

    // Validation: Koi box khali to nahi?
    if (!name || !code || !price || !stock) {
        alert("Please fill all required fields (Name, ID, Price, Stock)!");
        return;
    }

    // 2. Data ka bundle banao
    const productData = {
        name: name,
        category: category,
        code: code,
        ...
        cost_price: parseFloat(price) * 0.8 // Auto-calculate cost (Example: 80% of selling price)
    };
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Modal band karo aur page refresh karo
            document.getElementById('addProductModal').classList.add('hidden');
            location.reload(); // Taaki naya product list mein dikh jaye
        } else {
            const errorData = await response.json();
...
// --- Modal Scanner Logic ---
// --- Modal Scanner Logic (Optimized for Barcodes) ---
// --- ULTRA FAST MODAL SCANNER LOGIC ---
// --- IRIUN WEBCAM OPTIMIZED SCANNER ---
...
async function startModalScanner() {
    ...
    // ⚡ CONFIGURATION FOR IRIUN WEBCAM ⚡
    const config = {
        fps: 30,                  // High FPS to compensate for lag
        qrbox: { width: 250, height: 100 }, // Wide strip for barcodes
        aspectRatio: 1.77,        // 16:9 Ratio (Standard Phone Camera)
        disableFlip: false,       // Sometimes Iriun flips video, let library handle it
    ...
    try {
        // 🎯 Specific logic to find the BEST camera
        const devices = await Html5Qrcode.getCameras();
        
        if (devices && devices.length) {
            // Iriun Webcam usually has "Iriun" in its name. Let's find it!
            let iriunCamera = devices.find(device => device.label.toLowerCase().includes("iriun"));
            let selectedCameraId = iriunCamera ? iriunCamera.id : devices[0].id;
            ...
            await html5QrCode.start(
                ...
                (decodedText) => {
                    ...
                    // Visual feedback
                    document.getElementById('productID').style.backgroundColor = "#d1fae5"; // Light green
                    setTimeout(() => document.getElementById('productID').style.backgroundColor = "white", 1000);
                },
                () => {} // Ignore errors
            );
        } else {
